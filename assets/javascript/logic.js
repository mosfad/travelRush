...
$("#user-city").on("click", function(event) {
  ...
  var googleAPIKEY = "AIzaSyAur7oZgFWLDt_n5NWMl4dMjRDiqiirq-Q"; //new api key
  var qURL =
  ...
  $.ajax({
    ...
    console.log(
      "Coordinates for the input city: " +
        "Latitude = " +
        coordLoc.lat +
        " Longitude = " +
        coordLoc.long
...
function callAPI(term) {
  ...
  return $.ajax({
    url:
      `https://cors-anywhere.herokuapp.com/https://api.yelp.com/v3/businesses/search?term=${term}&latitude=` +
      coordLoc.lat +
      "&longitude=" +
      coordLoc.long +
      "&limit=10",
...
$("#user-input").on("click", function(event) {
    ...
    $.when(call1, call2, call3).done(function(y1, y2, y3) {
      if (searchTerm == "coffee") {
        ...
        for (i = 1; i < y3[0].businesses.length; i++) {
          ...
          var getMiles =
            Math.round(y3[0].businesses[i].distance * 0.000621371192 * 10) / 10;
          ...
          newLine.text(
            y3[0].businesses[i].name +
              " | " +
              "Price: " +
              y3[0].businesses[i].price +
              " | " +
              getMiles +
              " miles away"
          );
        ...
        for (i = 1; i < y1[0].businesses.length; i++) {
          //create var to store distance converted to miles
          var getMiles =
            Math.round(y1[0].businesses[i].distance * 0.000621371192 * 10) / 10;
          console.log(getMiles);
          ...
          $(newLine).attr("target", "_blank");
          newLine.text(
            y1[0].businesses[i].name +
              " | " +
              "Price: " +
              y1[0].businesses[i].price +
              " | " +
              getMiles +
              " miles away"
          );
        ...
        for (i = 1; i < y2[0].businesses.length; i++) {
          //create var to store distance converted to miles
          var getMiles =
            Math.round(y2[0].businesses[i].distance * 0.000621371192 * 10) / 10;
          console.log(getMiles);
          ...
          $(newLine).attr("target", "_blank");
          newLine.text(
            y2[0].businesses[i].name +
              " | " +
              "Price: " +
              y2[0].businesses[i].price +
              " | " +
              getMiles +
              " miles away"
          );
  ...
  $.ajax({
    ...
    for (var i = 0; i < listWeather.length; i++) {
      ...
      if (
        listWeather[i].dt_txt === convDateDay ||
        listWeather[i].dt_txt === convDateNight
      ) {
    ...
    for (var j = 0; j < indicesWeather.length; j++) {
      var urlIcon =
        ...
        listWeather[indicesWeather[j]].weather[0].icon +
        ".png";
      ...
      var rTemp =
        Math.round(listWeather[indicesWeather[j]].main.temp) + "&#8457";
      var rWind =
        Math.round(listWeather[indicesWeather[j]].wind.speed) + " mph";
...
$("#more-weather").on("click", function(event) {
  ...
  $.ajax({
    ...
    for (var i = 0; i < listWeather.length; i++) {
      console.log(listWeather[i].dt_txt);
      if (
        listWeather[i].dt_txt.includes("12:00") ||
        listWeather[i].dt_txt.includes("21:00")
      ) {
        console.log("found it!");
        indicesWeather.push(i);
